 /* UKHF Data checks
 Returns any organisations that have at least 1 discrepancy when compared with the UKHF table (0 denotes a discrepancy)
 The script will return 4 tables with discrepancies in the following order from 
 -Test 2 table
 -New Indicators table
 -Averages Table
 -Consultation Mediums table
 
 Please note this will only work for IAPT v2 data (Sept 2020 onwards)
 Made by JS 9/9/2021
 */
...
SET @MonthString = (DATENAME(m, @Period_start) + ' ' + CAST(DATEPART(yyyy, @Period_start) AS varchar))
PRINT @MonthString
...
SELECT Group_Type, Provider_code, Commissioner_Code, Measure_ID, Measure_Value
INTO #UKHF
FROM [NHSE_UKHF].[IAPT].[vw_Activity_Data1]
WHERE Effective_Snapshot_Date = @Period_End AND Commissioner_Code IS NOT NULL

-- Alter collation conflict
ALTER TABLE #UKHF
  ALTER COLUMN Group_Type
    VARCHAR(100) COLLATE Latin1_General_CI_AS NOT NULL
ALTER TABLE #UKHF
  ALTER COLUMN Commissioner_Code
    VARCHAR(100) COLLATE Latin1_General_CI_AS NOT NULL
ALTER TABLE #UKHF
...
PIVOT(
    SUM(Measure_Value)
    FOR Measure_ID IN([M001],[M031],[M076],[M019],[M020],[M021],[M029],[M030],[M024],[M025],[M026],[M027],[M032],[M033],[M034],[M035],[M056],[M057],[M058],[M059],[M060],[M061],[M071],
...
)_ WHERE [Referrals Match] = '0' OR [First Treatment Match] = '0' OR [Finished Treatment Match] = '0' OR [OpenReferralLessThan61DaysNoContact Match] = '0'
...
OR [NotCaseness Match] = '0' OR [ADSMFinishedTreatment Match] = '0' OR [CountAppropriatePairedADSM Match] = '0'
OR [SelfReferral Match] = '0'
...
[SelfReferral Match] = '0' OR [EndedBeforeTreatment Match] = '0' OR [FirstTreatment6Weeks Match] = '0' OR [FirstTreatment18Weeks Match] = '0' OR [WaitingForTreatment Match] = '0' OR [Appointments Match] = '0' OR [Appointments DNA Match]
= '0' OR [ReferralsEnded Match] = '0'-- OR [EndedSeenNotTreated Match] = '0' 
OR [EndedTreatedOnce Match] = '0' OR [Waiting2Weeks Match] = '0' OR [Waiting4Weeks Match] = '0' OR [Waiting6Weeks Match] = '0' OR [Waiting12Weeks Match] = '0' OR [Waiting18Weeks Match]
= '0' OR [FinishedCourseWait6Weeks Match] = '0' OR [FinishedCourseWait18Weeks Match] = '0' OR [FirstToSecondOver90Days Match] = '0' OR [RecoveryRate Match] = '0' OR [ReliableImprovementRate Match]
= '0' OR [DeteriorationRate Match] = '0' OR [NotCasenes Match] = '0' OR [FinishedCourseTreatment6WeeksRate Match] = '0' OR [FinishedCourseTreatment18WeeksRate Match] = '0' OR [ADSMCompletenessRate Match] = '0'
...
SELECT DISTINCT [Month],
	  ...
	  JOIN #UKHFPivot b ON (a.Level = Group_Type OR (a.Level = 'National' AND Group_Type = 'England') OR (a.Level = 'CCG/ Provider' AND Group_Type = 'CCG/Provider')) AND [CCG Code] = Commissioner_Code  AND [Provider Code] = Provider_Code 
WHERE Month = @MonthString AND Category = 'Total'
)_ WHERE [MedianApps Match] = '0' OR [MeanWait Match] = '0' OR [MedianWait Match] = '0' OR [MeanApps Match] = '0' OR [MeanFirstWaitFinished Match] = '0'
...
FROM [NHSE_Sandbox_MentalHealth].[dbo].[IAPT_Dashboard_Apps_Regions_ConsMed_Ethnicity2]
WHERE Month = @MonthString
GROUP BY Month,[Provider Code],[Provider Name],[CCG Code],[CCG Name],[Attendence Type] )_
...
PIVOT(
    SUM(Appointments)
    FOR [Attendence Type] IN(AptCancelledPatient,AptAttended,AptAttendedLate,AptLateNotSeen,AptDNA,AptCancelledProvider,Other
)) AS pivot_table 
...
JOIN #UKHFPivot b ON (a.Level = Group_Type OR (a.Level = 'National' AND Group_Type = 'England') OR (a.Level = 'CCG/ Provider' AND Group_Type = 'CCG/Provider')) AND [CCG Code] = Commissioner_Code  AND [Provider Code] = Provider_Code 
)_ WHERE [AptCancelledPatient Match] = '0' OR [AptAttended Match] = '0' OR [AptAttendedLate Match] = '0' OR [AptLateNotSeen Match] = '0' OR [AptDNA Match] = '0' OR [AptCancelledProvider Match] = '0'
