...
FROM	[mesh_IAPT].[IDS101referral] r
	...
	LEFT JOIN [Internal_Reference].[ComCodeChanges] cd ON r.OrgIDComm = cd.Org_Code COLLATE database_default
        LEFT JOIN [Internal_Hierarchies].[Commissioner_Hierarchies_E] ch ON COALESCE(cd.New_Code, r.OrgIDComm) = ch.Organisation_Code COLLATE database_default
	...
	LEFT JOIN [Internal_Hierarchies].[Provider_Hierarchies_E] ph ON COALESCE(ps.Prov_Successor, r.OrgID_Provider) = ph.Organisation_Code COLLATE database_default 
	--AND ph.Effective_To IS NULL --[Reporting].[Ref_ODS_Provider_Hierarchies_ICB]
	---------------------------
