...
-- WHEN UPDATING CHANGE THE BELOW TO RELEVANT MONTH
SET @Month = 'May 2022'
...
'Total' AS Category,
'Total' AS Variable,
SUM([OpenReferralLessThan61DaysNoContact]),
SUM([OpenReferral61-90DaysNoContact]),
SUM([OpenReferral91-120DaysNoContact]),
SUM([OpenReferral]),
SUM([Ended Treatment]),
SUM([Finished Treatment - 2 or more Apps]),
SUM([Referrals]),
SUM([EnteringTreatment]),
SUM([Waiting for Assessment]),
SUM([WaitingForAssessmentOver90Days]),
SUM([FirstAssessment28Days]),
SUM([FirstAssessment29to56Days]),
SUM([FirstAssessment57to90Days]),
SUM([FirstAssessmentOver90Days]),
SUM([FirstTreatment28days]),
SUM([FirstTreatment29to56days]),
SUM([FirstTreatment57to90days]),
SUM([FirstTreatmentOver90days]),
SUM([Ended Referral]),
SUM([Ended Not Suitable]),
SUM([Ended Signposted]),
SUM([Ended Mutual Agreement]),
SUM([Ended Referred Elsewhere]),
SUM([Ended Declined]),
SUM([Ended Deceased Assessed Only]),
SUM([Ended Unknown Assessed Only]),
SUM([Ended Stepped Up]),
SUM([Ended Stepped Down]),
SUM([Ended Completed]),
SUM([Ended Dropped Out]),
SUM([Ended Referred Non IAPT]),
SUM([Ended Deceased Treated]),
SUM([Ended Unknown Treated]),
SUM([Ended Invalid]),
SUM([Ended No Reason Recorded]),
SUM([Ended Seen Not Treated]),
SUM([Ended Treated Once]),
SUM([Ended Not Seen]),
SUM([Recovery]),
...
SUM([NotCaseness]),
SUM([ADSMFinishedTreatment]),
SUM([CountAppropriatePairedADSM]),
SUM([SelfReferral]),
SUM([GPReferral]),
SUM([OtherReferral]),
SUM([FirstToSecond28Days]),
SUM([FirstToSecond28To56Days]),
SUM([FirstToSecond57To90Days]),
SUM([FirstToSecondMoreThan90Days]),
SUM([Ended Not Assessed]),
SUM([Ended Incomplete Assessment]),
SUM([Ended Deceased (Seen but not taken on for a course of treatment)]),
SUM([Ended Not Known (Seen but not taken on for a course of treatment)]),
SUM([Ended Mutually agreed completion of treatment]),
SUM([Ended Termination of treatment earlier than Care Professional planned]),
SUM([Ended Termination of treatment earlier than patient requested]),
SUM([Ended Deceased (Seen and taken on for a course of treatment)]),
SUM([Ended Not Known (Seen and taken on for a course of treatment)]),
NULL AS RepeatReferrals,
SUM([RepeatReferrals2]),
'National' AS 'Level'



FROM [NHSE_Sandbox_MentalHealth].[dbo].[IAPT_Dashboard_Inequalities_Monthly_Region_Test_2]
WHERE Category = 'Total' AND Month = @Month
GROUP BY Month
...
SELECT MONTH,COUNT(*)
FROM NHSE_Sandbox_MentalHealth.[dbo].[IAPT_Dashboard_Monthly_IST_New_Indicators_Rounded]
GROUP BY Month
ORDER BY 1

--DELETE FROM NHSE_Sandbox_MentalHealth.[dbo].[IAPT_Dashboard_Monthly_IST_New_Indicators_Rounded]
...
'Total' AS Variable,
SUM(a.[SelfReferral]),
SUM([EndedBeforeTreatment]),
SUM([FirstTreatment2Weeks]),
SUM([FirstTreatment6Weeks]),
SUM([FirstTreatment12Weeks]),
SUM([FirstTreatment18Weeks]),
SUM([WaitingForTreatment]),
SUM([Appointments]),
SUM([Appointments DNA]),
SUM([ReferralsEnded]),
SUM([EndedTreatedOnce]),
SUM([Waiting2Weeks]),
SUM([Waiting4Weeks]),
SUM([Waiting6Weeks]),
SUM([Waiting12Weeks]),
SUM([Waiting18Weeks]),
SUM([FinishedCourseWait6Weeks]),
SUM([FinishedCourseWait18Weeks]),
SUM(a.[FirstToSecond28Days]),
SUM([FirstToSecond90Days]),
SUM([FirstToSecondOver90Days]),
...
ROUND(CASE WHEN SUM([Appointments DNA]) <5 OR SUM(Appointments) <5 then NULL ELSE (CAST(SUM([Appointments DNA])AS DECIMAL)/CAST(SUM(Appointments)AS DECIMAL)) END, 3) AS DNARate, 
...
ROUND(CASE WHEN SUM(FinishedCourseWait6Weeks) <5 OR SUM([Finished Treatment - 2 or more Apps]) <5   then NULL ELSE (CAST(SUM(FinishedCourseWait6Weeks)AS DECIMAL)/CAST(SUM([Finished Treatment - 2 or more Apps]) AS DECIMAL)) END, 3) AS FinishedCourseTreatment6WeeksRate ,
ROUND(CASE WHEN SUM(FinishedCourseWait18Weeks) <5 OR SUM([Finished Treatment - 2 or more Apps]) <5   then NULL ELSE (CAST(SUM(FinishedCourseWait18Weeks) AS DECIMAL)/CAST(SUM([Finished Treatment - 2 or more Apps]) AS DECIMAL)) END, 3) AS FinishedCourseTreatment18WeeksRate ,
...
ROUND(CASE WHEN SUM([FirstToSecondMoreThan90Days]) <5 OR SUM(a.[FirstToSecond28Days]+[FirstToSecond28To56Days]+[FirstToSecond57To90Days]+[FirstToSecondMoreThan90Days]) <5   then NULL ELSE (CAST(SUM([FirstToSecondMoreThan90Days]) AS DECIMAL)/CAST(SUM(a.[FirstToSecond28Days]+[FirstToSecond28To56Days]+[FirstToSecond57To90Days]+[FirstToSecondMoreThan90Days])AS DECIMAL)) END, 3) AS FirsttoSecond90daysRate

FROM NHSE_Sandbox_MentalHealth.[dbo].IAPT_Dashboard_Inequalities_Monthly_Region_Test_2_CCG a
LEFT JOIN NHSE_Sandbox_MentalHealth.[dbo].[IAPT_Dashboard_Inequalities_Monthly_IST_New_Indicators_CCG] b 
ON a.Month = b.Month AND a.[Provider Code]=b.[Provider Code] AND a.[CCG Code] = b.[CCG Code] AND a.[STP Code]=b.[STP Code] 
AND a.[Region Code] = b.[Region Code] AND a.Variable = b.Variable
...
CASE WHEN SUM([Appointments DNA])< 5 THEN NULL ELSE CAST(ROUND((SUM([Appointments DNA])+2) /5,0)*5 AS INT)  END AS [Appointments DNA],
...
CASE WHEN SUM(FinishedCourseWait6Weeks)< 5 THEN NULL ELSE CAST(ROUND((SUM(FinishedCourseWait6Weeks)+2) /5,0)*5 AS INT)  END AS FinishedCourseWait6Weeks,
CASE WHEN SUM(FinishedCourseWait18Weeks)< 5 THEN NULL ELSE CAST(ROUND((SUM(FinishedCourseWait18Weeks)+2) /5,0)*5 AS INT)  END AS FinishedCourseWait18Weeks,
...
CASE WHEN SUM(FirstToSecond90Days)< 5 THEN NULL ELSE CAST(ROUND((SUM(FirstToSecond90Days)+2) /5,0)*5 AS INT)  END AS FirstToSecond90Days,
CASE WHEN SUM(FirstToSecondOver90Days)< 5 THEN NULL ELSE CAST(ROUND((SUM(FirstToSecondOver90Days)+2) /5,0)*5 AS INT)  END AS FirstToSecondOver90Days,
NULL AS 'RightcareSimilarCCG',
'Region' AS 'Level',
...
ROUND(CASE WHEN SUM([Appointments DNA]) <5 OR SUM(Appointments) <5 then NULL ELSE (CAST(SUM([Appointments DNA])AS DECIMAL)/CAST(SUM(Appointments)AS DECIMAL)) END, 2) AS DNARate, 
...
ROUND(CASE WHEN SUM(FinishedCourseWait6Weeks) <5 OR SUM([Finished Treatment - 2 or more Apps]) <5   then NULL ELSE (CAST(SUM(FinishedCourseWait6Weeks)AS DECIMAL)/CAST(SUM([Finished Treatment - 2 or more Apps]) AS DECIMAL)) END, 2) AS FinishedCourseTreatment6WeeksRate ,
ROUND(CASE WHEN SUM(FinishedCourseWait18Weeks) <5 OR SUM([Finished Treatment - 2 or more Apps]) <5   then NULL ELSE (CAST(SUM(FinishedCourseWait18Weeks) AS DECIMAL)/CAST(SUM([Finished Treatment - 2 or more Apps]) AS DECIMAL)) END, 2) AS FinishedCourseTreatment18WeeksRate ,
...
ROUND(CASE WHEN SUM([FirstToSecondMoreThan90Days]) <5 OR SUM(a.[FirstToSecond28Days]+[FirstToSecond28To56Days]+[FirstToSecond57To90Days]+[FirstToSecondMoreThan90Days]) <5   then NULL ELSE (CAST(SUM([FirstToSecondMoreThan90Days]) AS DECIMAL)/CAST(SUM(a.[FirstToSecond28Days]+[FirstToSecond28To56Days]+[FirstToSecond57To90Days]+[FirstToSecondMoreThan90Days])AS DECIMAL)) END, 2) AS FirsttoSecond90daysRate
...
LEFT JOIN NHSE_Sandbox_MentalHealth.[dbo].RightcareSimilar10 b ON a.[CCG Code] = b.[CCG Code] 
AND a.[CCG Name] = b.[CCG Name]
WHERE Level = 'CCG'

----TRUNCATE TABLE NHSE_Sandbox_MentalHealth.[dbo].[IAPT_Dashboard_Region_Monthly_Test_2_Rounded]
--ALTER TABLE NHSE_Sandbox_MentalHealth.[dbo].[IAPT_Dashboard_Region_Monthly_Test_2_Rounded]
--ADD Level varchar(500)

--SELECT MONTH,COUNT(*)
--FROM NHSE_Sandbox_MentalHealth.[dbo].[IAPT_Over_65_Metrics]
--GROUP BY Month
--ORDER BY 1

--USE [NHSE_Sandbox_MentalHealth];
--GO
--EXEC sp_rename 'IAPT_Dashboard_Averages_IST', 'IAPT_Dashboard_Averages_IST_ICB'
--EXEC sp_rename 'IAPT_Dashboard_Averages_IST_CCG', 'IAPT_Dashboard_Averages_IST';

--DELETE FROM NHSE_Sandbox_MentalHealth.[dbo].[IAPT_Dashboard_Appointment_Types_Totals]
--WHERE Month = 'March 2022'
...
FROM [NHSE_Sandbox_MentalHealth].[dbo].[IAPT_Dashboard_Apps_Primary_Loop_CCG]
WHERE Category = 'Total' AND Month = @Month
GROUP BY Month
...
CASE WHEN [IPTApts]< 5 THEN NULL ELSE CAST(ROUND(([IPTApts]+2) /5,0)*5 AS INT)  END,
CASE WHEN [ESApts]< 5 THEN NULL ELSE CAST(ROUND(([ESApts]+2) /5,0)*5 AS INT)  END,
CASE WHEN [Signposting]< 5 THEN NULL ELSE CAST(ROUND(([Signposting]+2) /5,0)*5 AS INT)  END)_