...
INTO [MHDInternal].[TEMP_TTAD_PDT_IETDuration]
FROM(
	SELECT DISTINCT
		i.PathwayID
...
-- Create base table: [MHDInternal].[TEMP_TTAD_PDT_IETBase] --------------------------------------------
...
SELECT DISTINCT
	...
	,[IntEnabledTherProg] -- this is just here to check the case when statement is right
	,CASE WHEN ServDischDate IS NOT NULL AND CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN l.ReportingPeriodStartDate AND l.ReportingPeriodEndDate AND r.[PathwayID] IS NOT NULL THEN 1 ELSE 0
	END AS 'Finished Treatment - 2 or more Apps'
	,CASE WHEN ServDischDate IS NOT NULL AND CompletedTreatment_Flag = 'True' AND Recovery_Flag = 'True' AND r.ServDischDate BETWEEN l.ReportingPeriodStartDate AND l.ReportingPeriodEndDate AND r.[PathwayID] IS NOT NULL THEN 1 ELSE 0
	END AS 'Recovery'
	,CASE WHEN ServDischDate IS NOT NULL AND CompletedTreatment_Flag = 'True' AND ReliableImprovement_Flag = 'True' AND Recovery_Flag = 'True' AND r.ServDischDate BETWEEN l.ReportingPeriodStartDate AND l.ReportingPeriodEndDate AND r.[PathwayID] IS NOT NULL THEN 1 ELSE 0 
	END AS 'Reliable Recovery'
	,CASE WHEN ServDischDate IS NOT NULL AND CompletedTreatment_Flag = 'True' AND NoChange_Flag = 'True' AND r.ServDischDate BETWEEN l.ReportingPeriodStartDate AND l.ReportingPeriodEndDate AND r.[PathwayID] IS NOT NULL THEN 1 ELSE 0
	END AS 'No Change'
	,CASE WHEN ServDischDate IS NOT NULL AND CompletedTreatment_Flag = 'True' AND ReliableDeterioration_Flag = 'True' AND r.ServDischDate BETWEEN l.ReportingPeriodStartDate AND l.ReportingPeriodEndDate AND r.[PathwayID] IS NOT NULL THEN 1 ELSE 0
	END AS 'Reliable Deterioration'
	,CASE WHEN ServDischDate IS NOT NULL AND CompletedTreatment_Flag = 'True' AND ReliableImprovement_Flag = 'True' AND r.ServDischDate BETWEEN l.ReportingPeriodStartDate AND l.ReportingPeriodEndDate AND r.[PathwayID] IS NOT NULL THEN 1 ELSE 0
	END AS 'Reliable Improvement'
	,CASE WHEN ServDischDate IS NOT NULL AND CompletedTreatment_Flag = 'True' AND NotCaseness_Flag = 'True' AND r.ServDischDate BETWEEN l.ReportingPeriodStartDate AND l.ReportingPeriodEndDate AND r.[PathwayID] IS NOT NULL THEN 1 ELSE 0
	END AS 'NotCaseness'
...
FROM	[mesh_IAPT].[IDS101referral] r
		...
		--------------------------
		LEFT JOIN [MHDInternal].[TEMP_TTAD_PDT_IETDuration] iet ON r.[PathwayID] = iet.[PathwayID] AND ROWID = 1
		---------------------------
...
WHERE	
	r.UsePathway_Flag = 'True' AND l.IsLatest = 1 
	AND r.ServDischDate BETWEEN l.[ReportingPeriodStartDate] AND l.[ReportingPeriodEndDate]
	AND r.CompletedTreatment_Flag='True'
	AND l.[ReportingPeriodStartDate] BETWEEN DATEADD(MONTH,0, @PeriodStart) AND @PeriodStart --For monthly refresh the offset should be 0 for the latest month only

---- Final aggregate table: [MHDInternal].[DASHBOARD_TTAD_PDT_IET] ----------------------------------------------------
--IF OBJECT_ID('[MHDInternal].[DASHBOARD_TTAD_PDT_IET]') IS NOT NULL DROP TABLE [MHDInternal].[DASHBOARD_TTAD_PDT_IET]
...
--INTO [MHDInternal].[DASHBOARD_TTAD_PDT_IET] 
...
DROP TABLE [MHDInternal].[TEMP_TTAD_PDT_IETDuration]
