...
SELECT DISTINCT
	...
	,CASE WHEN a.CareContDate BETWEEN l.ReportingPeriodStartDate AND l.ReportingPeriodEndDate AND (a.ConsMechanism IN ('01', '1', '1 ', ' 1') OR a.ConsMediumUsed IN ('01', '1', '1 ', ' 1')) AND a.Unique_CareContactID IS NOT NULL THEN 1 ELSE 0
	END AS 'Face to face communication' --in both v2.0 and v2.1
	,CASE WHEN a.CareContDate BETWEEN l.ReportingPeriodStartDate AND l.ReportingPeriodEndDate AND (a.ConsMechanism IN ('02', '2', '2 ', ' 2') OR a.ConsMediumUsed IN ('02', '2', '2 ', ' 2')) AND a.Unique_CareContactID IS NOT NULL THEN 1 ELSE 0
	END AS 'Telephone' --in both v2.0 and v2.1
	,CASE WHEN a.CareContDate BETWEEN l.ReportingPeriodStartDate AND l.ReportingPeriodEndDate AND (a.ConsMechanism IN ('03', '3', '3 ', ' 3') OR a.ConsMediumUsed IN ('03', '3', '3 ', ' 3')) AND a.Unique_CareContactID IS NOT NULL THEN 1 ELSE 0
	END AS 'Telemedicine web camera' --just in v2.0
	,CASE WHEN a.CareContDate BETWEEN l.ReportingPeriodStartDate AND l.ReportingPeriodEndDate AND (a.ConsMechanism IN ('04', '4', '4 ', ' 4') OR a.ConsMediumUsed IN ('04', '4', '4 ', ' 4')) AND a.Unique_CareContactID IS NOT NULL THEN 1 ELSE 0
	END AS 'Talk type for a Person unable to speak' --in both v2.0 and v2.1
	,CASE WHEN a.CareContDate BETWEEN l.ReportingPeriodStartDate AND l.ReportingPeriodEndDate AND (a.ConsMechanism IN ('05', '5', '5 ', ' 5') OR a.ConsMediumUsed IN ('05', '5', '5 ', ' 5')) AND a.Unique_CareContactID IS NOT NULL THEN 1 ELSE 0
	END AS 'Email' --in both v2.0 and v2.1
	,CASE WHEN a.CareContDate BETWEEN l.ReportingPeriodStartDate AND l.ReportingPeriodEndDate AND (a.ConsMechanism IN ('06', '6', '6 ', ' 6') OR a.ConsMediumUsed IN ('06', '6', '6 ', ' 6')) AND a.Unique_CareContactID IS NOT NULL THEN 1 ELSE 0
	END AS 'Short Message Service (SMS)' --in just v2.0
	,CASE WHEN a.CareContDate BETWEEN l.ReportingPeriodStartDate AND l.ReportingPeriodEndDate AND (a.ConsMechanism IN ('98', '98 ', ' 98') OR a.ConsMechanism IS NULL OR a.ConsMediumUsed IN ('98', '98 ', ' 98') OR a.ConsMediumUsed IS NULL) AND a.Unique_CareContactID IS NOT NULL THEN 1 ELSE 0
	END AS 'Other' --in both v2.0 and v2.1
	,CASE WHEN a.CareContDate BETWEEN l.ReportingPeriodStartDate AND l.ReportingPeriodEndDate AND (a.ConsMechanism IN ('08', '8', '8 ', ' 8') OR a.ConsMediumUsed IN ('08', '8', '8 ', ' 8')) AND a.Unique_CareContactID IS NOT NULL THEN 1 ELSE 0
	END AS 'Online Instant Messaging' --in just v2.0
	,CASE WHEN a.CareContDate BETWEEN l.ReportingPeriodStartDate AND l.ReportingPeriodEndDate AND (a.ConsMechanism IN ('09', '9', '9 ', ' 9') OR a.ConsMediumUsed IN ('09', '9', '9 ', ' 9')) AND a.Unique_CareContactID IS NOT NULL THEN 1 ELSE 0
	END AS 'Text Message (Asynchronous)' --in just v2.1
	,CASE WHEN a.CareContDate BETWEEN l.ReportingPeriodStartDate AND l.ReportingPeriodEndDate AND (a.ConsMechanism IN ('10', '10 ', ' 10') OR a.ConsMediumUsed IN ('10', '10 ', ' 10')) AND a.Unique_CareContactID IS NOT NULL THEN 1 ELSE 0
	END AS 'Instant messaging (Synchronous)' --in just v2.1
	,CASE WHEN a.CareContDate BETWEEN l.ReportingPeriodStartDate AND l.ReportingPeriodEndDate AND (a.ConsMechanism IN ('11', '11 ', ' 11') OR a.ConsMediumUsed IN ('11', '11 ', ' 11')) AND a.Unique_CareContactID IS NOT NULL THEN 1 ELSE 0
	END AS 'Video consultation' --in just v2.1
	,CASE WHEN a.CareContDate BETWEEN l.ReportingPeriodStartDate AND l.ReportingPeriodEndDate AND (a.ConsMechanism IN ('12', '12 ', ' 12') OR a.ConsMediumUsed IN ('12', '12 ', ' 12')) AND a.Unique_CareContactID IS NOT NULL THEN 1 ELSE 0
	END AS 'Message Board (Asynchronous)' --in just v2.1
	,CASE WHEN a.CareContDate BETWEEN l.ReportingPeriodStartDate AND l.ReportingPeriodEndDate AND (a.ConsMechanism IN ('13', '13 ', ' 13') OR a.ConsMediumUsed IN ('13', '13 ', ' 13')) AND a.Unique_CareContactID IS NOT NULL THEN 1 ELSE 0
