...
SELECT DISTINCT
	...
	,CASE WHEN ch.[Organisation_Code] IS NULL OR ch.[Organisation_Name] LIKE '%REPORTING ENTITY%' THEN 'Other' ELSE ch.[Organisation_Code] END AS 'CCG Code'
	,CASE WHEN ch.[Organisation_Name] IS NULL OR ch.[Organisation_Name] LIKE '%REPORTING ENTITY%' THEN 'Other' ELSE ch.Organisation_Name END AS 'CCG Name' 
...
LEFT JOIN [Internal_Hierarchies].[Commissioner_Hierarchies_TCUBE] ch ON COALESCE(cc.New_Code, rr.[LatestCCG]) = ch.Organisation_Code COLLATE database_default --[Reporting].[Ref_ODS_Commissioner_Hierarchies_ICB]
	AND ch.Region_Name <> 'Wales Region'              --AND ch.Effective_To IS NULL

LEFT JOIN [Internal_Reference].[Provider_Successor] ps ON rr.[LatestProvider] = ps.Prov_original COLLATE database_default
LEFT JOIN [Internal_Hierarchies].[Provider_Hierarchies_TCUBE] ph ON COALESCE(ps.Prov_Successor, rr.[LatestProvider]) = ph.Organisation_Code COLLATE database_default --[Reporting].[Ref_ODS_Provider_Hierarchies_ICB]
	--AND ph.Effective_To IS NULL
