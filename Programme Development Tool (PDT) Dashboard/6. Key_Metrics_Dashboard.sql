...
FROM	[mesh_IAPT].[IDS101referral] r
		...
		--LEFT JOIN [MHDInternal].[TEMP_TTAD_PlannedAccessNumbers] pa ON r.OrgIDComm = pa.CCG AND i.ReportingPeriodStartDate = pa.[Month]
...
---ADDING Average Waits and Appointments data tables
--Run for past 12 months for ICBs
...
SELECT MAX([Month]) AS MaxMonth
	  INTO [MHDInternal].[Temp_TTAD_MaxMonth]
FROM [MHDInternal].[DASHBOARD_TTAD_PDT_Avg_Max_Wait]
...
FROM [MHDInternal].[DASHBOARD_TTAD_PDT_Avg_AssessToFirstLIHI] hl INNER JOIN 
[MHDInternal].[DASHBOARD_TTAD_PDT_Avg_Max_Wait] mw ON hl.[CCG Code] = mw.[CCG Code] AND hl.Month = mw.Month AND hl.Level = mw.Level
INNER JOIN 
[MHDInternal].[DASHBOARD_TTAD_PDT_Avg_Wait_Between_Apts] aw ON hl.[CCG Code] = aw.[CCG Code] AND hl.Month = aw.Month AND hl.Level = aw.Level
INNER JOIN 
[MHDInternal].[DASHBOARD_TTAD_Averages] av ON hl.[CCG Code] = av.[CCG Code] AND hl.Month = av.month AND av.Level IN ('CCG','National') AND Category = 'Total'
LEFT JOIN
[MHDInternal].[Temp_TTAD_MaxMonth] mm ON hl.Month = mm.MaxMonth AND DATEDIFF(M, hl.[Month], MaxMonth) <= 12
LEFT JOIN [Internal_Reference].[ComCodeChanges] cc ON hl.[CCG Code] = cc.Org_Code COLLATE database_default
LEFT JOIN [Reporting].[Ref_ODS_Commissioner_Hierarchies_ICB] ch ON COALESCE(cc.New_Code, hl.[CCG Code]) = ch.Organisation_Code COLLATE database_default
			AND ch.Effective_To IS NULL
