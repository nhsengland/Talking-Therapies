...
FROM(
	SELECT
		...
		,IntEnabledTherProg
		,SUM(DurationIntEnabledTher) AS DurationIntEnabledTher
		,SUM(Count_IET) AS Count_IET
	FROM [MHDInternal].[TEMP_TTAD_IET_TypeAndDuration_Step1]
	GROUP BY 
		PathwayID
		,IntEnabledTherProg
) a
...
SELECT 
	...
	,IntegratedSoftwareInd
	,SUM(CompTreatFlag) AS CompTreatFlag
INTO [MHDInternal].[DASHBOARD_TTAD_IET_IntegSoft]
FROM [MHDInternal].[TEMP_TTAD_IET_Base]
...
-- Create base table with one record per row which is then aggregated to produce [MHDInternal].[DASHBOARD_TTAD_IET_PEQ]

-- For refresh, the offset for period start and end should be 0 to get the latest month
...
-- For quarterly refresh the @Offset should be set to -3
