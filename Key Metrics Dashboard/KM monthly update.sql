...
FROM [MHDInternal].[DASHBOARD_TTAD_PDT_Avg_AssessToFirstLIHI] hl
     ...
     -----------------------
      ...
      LEFT JOIN [Internal_Hierarchies].[Commissioner_Hierarchies_TCUBE] ch ON COALESCE(cc.New_Code, hl.[CCG Code]) = ch.Organisation_Code COLLATE database_default 
	  AND ch.Region_Name <> 'Wales Region' AND (ch.Effective_To IS NULL OR ch.Effective_To >= hl.[Month]) --[Reporting].[Ref_ODS_Commissioner_Hierarchies_ICB]
