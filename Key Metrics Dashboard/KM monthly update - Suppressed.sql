-- DELETE MAX(Month) -----------------------------------------------------------------------
DELETE FROM [MHDInternal].[DASHBOARD_TTAD_KeyMetrics_Suppressed]
WHERE [Month] = (SELECT MAX([Month]) FROM [MHDInternal].[DASHBOARD_TTAD_KeyMetrics_Suppressed])
...
INSERT INTO MHDInternal.DASHBOARD_TTAD_KeyMetrics_Suppressed																		
...
FROM (																					
  ...
  SELECT																					
    ...
    -------------
		,CASE 
			WHEN COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END) = 0 
				THEN NULL																			
			WHEN COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND ReliableImprovement_Flag = 'True' THEN  r.PathwayID ELSE NULL END) = 0
				THEN NULL 																					
			ELSE 																																				
...
SELECT																					
		...
		,CASE
			WHEN COUNT(DISTINCT CASE WHEN ReferralRequestReceivedDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END) < 5 
				THEN NULL
			ELSE
				ROUND((COUNT(DISTINCT CASE WHEN ReferralRequestReceivedDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END)/5.0),0)*5 
			END AS 'Count_Referrals'		
																					
		,CASE
			WHEN COUNT(DISTINCT CASE WHEN TherapySession_FirstDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END) < 5
				THEN NULL
			ELSE
				ROUND((COUNT(DISTINCT CASE WHEN TherapySession_FirstDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END)/5.0),0)*5
			END AS 'Count_Access'																			
																					
		,CASE
			WHEN COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND  Recovery_Flag = 'True' THEN  r.PathwayID ELSE NULL END) < 5
				THEN NULL
			ELSE
				ROUND((COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND  Recovery_Flag = 'True' THEN  r.PathwayID ELSE NULL END)/5.0),0)*5 
			END AS 'Count_Recovery'																		
																					
		,CASE 
			WHEN COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END) < 5
				THEN NULL
			ELSE
				ROUND((COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END)/5.0),0)*5  
			END AS 'Count_FinishedCourseTreatment'																			
																					
		,CASE
			WHEN COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND NotCaseness_Flag = 'True' THEN r.PathwayID ELSE NULL END) < 5
				THEN NULL
			ELSE
				ROUND((COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND NotCaseness_Flag = 'True' THEN r.PathwayID ELSE NULL END)/5.0),0)*5 
			END AS 'Count_NotAtCaseness'	
				
		,CASE
			WHEN COUNT(DISTINCT CASE WHEN r.[ServDischDate] BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND r.[TherapySession_FirstDate] IS NULL THEN r.[PathwayID] ELSE NULL END) < 5
				THEN NULL
			ELSE
				ROUND((COUNT(DISTINCT CASE WHEN r.[ServDischDate] BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND r.[TherapySession_FirstDate] IS NULL THEN r.[PathwayID] ELSE NULL END)/5.0),0)*5 
			END AS 'EndedBeforeTreatment'	

		,CASE
			WHEN COUNT(DISTINCT CASE WHEN r.[ServDischDate] BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND r.[TreatmentCareContact_Count] = 1 THEN r.[PathwayID] ELSE NULL END) < 5
				THEN NULL
			ELSE
				ROUND((COUNT(DISTINCT CASE WHEN r.[ServDischDate] BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND r.[TreatmentCareContact_Count] = 1 THEN r.[PathwayID] ELSE NULL END)/5.0),0)*5 
			END AS 'EndedTreatedOnce'
																					
		,CASE 
			WHEN COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END)																			
					-COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND NotCaseness_Flag = 'True' THEN r.PathwayID ELSE NULL END) < 5 
				THEN NULL																			
			WHEN COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND  Recovery_Flag = 'True' THEN  r.PathwayID ELSE NULL END) < 5 
				THEN NULL 																			
			ELSE 																																							
				(CAST(COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND Recovery_Flag = 'True' THEN  r.PathwayID ELSE NULL END) AS float)																			
				/(CAST(COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END) AS float)																			
					-CAST(COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND NotCaseness_Flag = 'True' THEN r.PathwayID ELSE NULL END)AS float))) 
			END AS 'Percentage_Recovery'
		
		,CASE 
			WHEN COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END)																			
					-COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND NotCaseness_Flag = 'True' THEN r.PathwayID ELSE NULL END) < 5 
				THEN NULL																			
			WHEN COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND ReliableImprovement_Flag = 'True' AND Recovery_Flag = 'True' THEN  r.PathwayID ELSE NULL END) < 5 
				THEN NULL 																			
			ELSE 																																				
				(CAST(COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND ReliableImprovement_Flag = 'True' AND Recovery_Flag = 'True' THEN  r.PathwayID ELSE NULL END) AS float)																			
				/(CAST(COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END) AS float)																			
		...
		,CASE 
			WHEN COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END) < 5 
				THEN NULL																			
			WHEN COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND ReliableDeterioration_Flag = 'True' THEN  r.PathwayID ELSE NULL END) < 5
				THEN NULL 																			
			ELSE 																																								
				(CAST(COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND ReliableDeterioration_Flag = 'True' THEN  r.PathwayID ELSE NULL END) AS float)																			
				/(CAST(COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END) AS float))) END																				
		...
		,CASE 
			WHEN COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END) < 5
				THEN NULL																			
			WHEN COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND ReliableImprovement_Flag = 'True' THEN  r.PathwayID ELSE NULL END) < 5
				THEN NULL 																					
			ELSE 																																					
				(CAST(COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND ReliableImprovement_Flag = 'True' THEN  r.PathwayID ELSE NULL END) AS float)																			
				/(CAST(COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END) AS float))) END																			
		...
		,CASE 
			WHEN COUNT(DISTINCT CASE WHEN Validated_EthnicCategory = 'A' AND CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END)																			
					-COUNT(DISTINCT CASE WHEN Validated_EthnicCategory = 'A' AND CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND NotCaseness_Flag = 'True' THEN r.PathwayID ELSE NULL END) < 5 
				THEN NULL																			
			WHEN COUNT(DISTINCT CASE WHEN Validated_EthnicCategory = 'A' AND CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND  Recovery_Flag = 'True' THEN  r.PathwayID ELSE NULL END) < 5
				THEN NULL 																				
			ELSE 																			
																						
				(CAST(COUNT(DISTINCT CASE WHEN Validated_EthnicCategory = 'A' AND CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND  Recovery_Flag = 'True' THEN  r.PathwayID ELSE NULL END) AS float)																			
				/(CAST(COUNT(DISTINCT CASE WHEN Validated_EthnicCategory = 'A' AND CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END) AS float)																			
					-CAST(COUNT(DISTINCT CASE WHEN Validated_EthnicCategory = 'A' AND CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND NotCaseness_Flag = 'True' THEN r.PathwayID ELSE NULL END)AS float)))																	
			END AS 'Percentage_Recovery_WB'

		,CASE 
			WHEN COUNT(DISTINCT CASE WHEN Validated_EthnicCategory IN ('B','C','D','E','F','G','B','C','H','J','K','L''M','N','P','R','S') AND CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END)																			
					-COUNT(DISTINCT CASE WHEN Validated_EthnicCategory IN ('B','C','D','E','F','G','B','C','H','J','K','L''M','N','P','R','S') AND CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND NotCaseness_Flag = 'True' THEN r.PathwayID ELSE NULL END) < 5
				THEN NULL																			
			WHEN COUNT(DISTINCT CASE WHEN Validated_EthnicCategory IN ('B','C','D','E','F','G','B','C','H','J','K','L''M','N','P','R','S') AND CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND  Recovery_Flag = 'True' THEN  r.PathwayID ELSE NULL END) < 5
				THEN NULL 																					
			ELSE 																																								
				(CAST(COUNT(DISTINCT CASE WHEN Validated_EthnicCategory IN ('B','C','D','E','F','G','B','C','H','J','K','L''M','N','P','R','S') AND CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND  Recovery_Flag = 'True' THEN  r.PathwayID ELSE NULL END) AS float)																			
				/(CAST(COUNT(DISTINCT CASE WHEN Validated_EthnicCategory IN ('B','C','D','E','F','G','B','C','H','J','K','L''M','N','P','R','S') AND CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END) AS float)																			
					-CAST(COUNT(DISTINCT CASE WHEN Validated_EthnicCategory IN ('B','C','D','E','F','G','B','C','H','J','K','L''M','N','P','R','S') AND CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND NotCaseness_Flag = 'True' THEN r.PathwayID ELSE NULL END)AS float)))																			
			END AS 'Percentage_Recovery_EM'
																													
		,CASE
			WHEN COUNT( DISTINCT CASE WHEN r.[ServDischDate] BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND CompletedTreatment_Flag = 'True' AND 
					DATEDIFF(dd,[ReferralRequestReceivedDate],[TherapySession_FirstDate]) <=42 THEN r.PathwayID ELSE NULL END) < 5
				THEN NULL
			ELSE
				ROUND((COUNT( DISTINCT CASE WHEN r.[ServDischDate] BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND CompletedTreatment_Flag = 'True' AND 
					DATEDIFF(dd,[ReferralRequestReceivedDate],[TherapySession_FirstDate]) <=42 THEN r.PathwayID ELSE NULL END)/5.0),0)*5 
			END AS 'Count_FirstTreatment6WeeksFinishedCourseTreatment'																		
																					
		,CASE
			WHEN COUNT( DISTINCT CASE WHEN r.[ServDischDate] BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND CompletedTreatment_Flag = 'True' AND 
					DATEDIFF(dd,[ReferralRequestReceivedDate],[TherapySession_FirstDate])<=126 THEN r.PathwayID ELSE NULL END) < 5
				THEN NULL
			ELSE
				ROUND((COUNT( DISTINCT CASE WHEN r.[ServDischDate] BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND CompletedTreatment_Flag = 'True' AND 
					DATEDIFF(dd,[ReferralRequestReceivedDate],[TherapySession_FirstDate])<=126 THEN r.PathwayID ELSE NULL END)/5.0),0)*5 
			END AS 'Count_FirstTreatment18WeeksFinishedCourseTreatment'																			
																					
		,CASE 
			WHEN COUNT(DISTINCT CASE WHEN r.[ServDischDate] BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND CompletedTreatment_Flag = 'True' THEN r.PathwayID ELSE NULL END) < 5 
				THEN NULL																			
			WHEN COUNT(DISTINCT CASE WHEN r.[ServDischDate] BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND CompletedTreatment_Flag = 'True' AND DATEDIFF(dd,[ReferralRequestReceivedDate],[TherapySession_FirstDate])																			
				<=42 THEN r.PathwayID ELSE NULL END) < 5 
				THEN NULL 																		
			ELSE 																																					
				(CAST(COUNT(DISTINCT CASE WHEN r.[ServDischDate] BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND CompletedTreatment_Flag = 'True' AND DATEDIFF(dd,[ReferralRequestReceivedDate],[TherapySession_FirstDate])																			
					<=42 THEN r.PathwayID ELSE NULL END) AS float)																		
				/(CAST(COUNT(DISTINCT CASE WHEN r.[ServDischDate] BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND CompletedTreatment_Flag = 'True' THEN r.PathwayID ELSE NULL END) AS float)))																			
			END AS 'Percentage_FirstTreatment6WeeksFinishedCourseTreatment'																	
																					
		,CASE 
			WHEN COUNT(DISTINCT CASE WHEN r.[ServDischDate] BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND CompletedTreatment_Flag = 'True' THEN r.PathwayID ELSE NULL END) < 5 
				THEN NULL																			
			WHEN COUNT(DISTINCT CASE WHEN r.[ServDischDate] BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND CompletedTreatment_Flag = 'True' AND DATEDIFF(dd,[ReferralRequestReceivedDate],[TherapySession_FirstDate])																			
				<=126 THEN r.PathwayID ELSE NULL END) < 5 
				THEN NULL 																		
			ELSE 																																					
				(CAST(COUNT( DISTINCT CASE WHEN r.[ServDischDate] BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND CompletedTreatment_Flag = 'True' AND DATEDIFF(dd,[ReferralRequestReceivedDate],[TherapySession_FirstDate])																			
					<=126 THEN r.PathwayID ELSE NULL END) AS float)																		
				/(CAST(COUNT(DISTINCT CASE WHEN r.[ServDischDate] BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND CompletedTreatment_Flag = 'True' THEN r.PathwayID ELSE NULL END) AS float)))																			
			END AS 'Percentage_FirstTreatment18WeeksFinishedCourseTreatment'																
																					
		,CASE
			WHEN COUNT(DISTINCT CASE WHEN R.[TherapySession_SecondDate] BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND DATEDIFF(DD,[TherapySession_FirstDate],[TherapySession_SecondDate]) >28 THEN r.PathwayID ELSE NULL END) < 5
				THEN NULL
			ELSE
				ROUND((COUNT(DISTINCT CASE WHEN R.[TherapySession_SecondDate] BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND DATEDIFF(DD,[TherapySession_FirstDate],[TherapySession_SecondDate]) >28 THEN r.PathwayID ELSE NULL END)/5.0),0)*5 		
			END AS 'Count_FirstToSecondTreatmentOver28days'															

		,CASE
			WHEN COUNT(DISTINCT CASE WHEN R.[TherapySession_SecondDate] BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND DATEDIFF(DD,[TherapySession_FirstDate],[TherapySession_SecondDate]) >90 THEN r.PathwayID ELSE NULL END) < 5
				THEN NULL
			ELSE
				ROUND((COUNT(DISTINCT CASE WHEN R.[TherapySession_SecondDate] BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND DATEDIFF(DD,[TherapySession_FirstDate],[TherapySession_SecondDate]) >90 THEN r.PathwayID ELSE NULL END)/5.0),0)*5  
			END AS 'Count_FirstToSecondTreatmentOver90days'																		
																					
		,CASE
			WHEN COUNT(DISTINCT CASE WHEN TherapySession_SecondDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END) < 5
				THEN NULL
			ELSE
				ROUND((COUNT(DISTINCT CASE WHEN TherapySession_SecondDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END)/5.0),0)*5 
			END AS 'Count_SecondTreatment'																	
																					
		,CASE 
			WHEN COUNT(DISTINCT CASE WHEN  TherapySession_SecondDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END) < 5 
				THEN NULL																			
			WHEN COUNT(DISTINCT CASE WHEN TherapySession_SecondDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND DATEDIFF(DD,[TherapySession_FirstDate],[TherapySession_SecondDate]) >90 THEN r.PathwayID ELSE NULL END) < 5 
				THEN NULL 																			
			ELSE 																																					
				(CAST(COUNT( DISTINCT CASE WHEN TherapySession_SecondDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND DATEDIFF(DD,[TherapySession_FirstDate],[TherapySession_SecondDate]) >90 THEN r.PathwayID ELSE NULL END) AS float)																			
				/(CAST(COUNT(DISTINCT CASE WHEN TherapySession_SecondDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] THEN r.PathwayID ELSE NULL END) AS float)))																			
			END AS 'Percentage_FirstToSecondTreatmentOver90days'																	
																					
		,CASE
			WHEN COUNT(DISTINCT CASE WHEN r.ServDischDate IS NULL AND DATEDIFF(DD ,TherapySession_LastDate, [ReportingPeriodEndDate])  >90 THEN r.PathwayID ELSE NULL END) < 5
				THEN NULL
			ELSE
				ROUND((COUNT(DISTINCT CASE WHEN r.ServDischDate IS NULL AND DATEDIFF(DD ,TherapySession_LastDate, [ReportingPeriodEndDate])  >90 THEN r.PathwayID ELSE NULL END)/5.0),0)*5 
			END AS 'OpenReferralOver90daysNoContact'																			
	
		,CASE
			WHEN COUNT(DISTINCT CASE WHEN r.ServDischDate IS NULL AND TherapySession_LastDate IS NOT NULL  THEN r.PathwayID ELSE NULL END) < 5
				THEN NULL
			ELSE
				ROUND((COUNT(DISTINCT CASE WHEN r.ServDischDate IS NULL AND TherapySession_LastDate IS NOT NULL  THEN r.PathwayID ELSE NULL END)/5.0),0)*5 
			END AS 'OpenReferral'																		
		...
		,CASE 
			...
			ELSE 																																								
				(CAST(COUNT(DISTINCT CASE WHEN r.ServDischDate IS NULL AND DATEDIFF(DD ,TherapySession_LastDate, [ReportingPeriodEndDate])  >90 THEN r.PathwayID ELSE NULL END) AS float)																			
				/(CAST(COUNT(DISTINCT CASE WHEN r.ServDischDate IS NULL AND TherapySession_LastDate IS NOT NULL  THEN r.PathwayID ELSE NULL END) AS float)))																			
			END AS 'Percentage_OpenReferralNoActivityOver90days'

		,CASE 
			WHEN COUNT( DISTINCT CASE WHEN r.ServDischDate IS NULL AND TherapySession_LastDate IS NOT NULL  THEN r.PathwayID ELSE NULL END) < 5 
				THEN NULL																			
			WHEN COUNT( DISTINCT CASE WHEN r.ServDischDate IS NULL AND TreatmentCareContact_Count > 1  AND DATEDIFF(DD ,TherapySession_LastDate, [ReportingPeriodEndDate])  >90 THEN r.PathwayID ELSE NULL END) < 5 
				THEN NULL 																			
			ELSE 																																								
				(CAST(COUNT( DISTINCT CASE WHEN r.ServDischDate IS NULL AND TreatmentCareContact_Count > 1 AND DATEDIFF(DD ,TherapySession_LastDate, [ReportingPeriodEndDate])  >90 THEN r.PathwayID ELSE NULL END) AS float)																			
				/(CAST(COUNT( DISTINCT CASE WHEN r.ServDischDate IS NULL AND TherapySession_LastDate IS NOT NULL  THEN r.PathwayID ELSE NULL END) AS float)))																			
			END AS 'Percentage_OpenReferralNoActivityOver90days2Apps'																	
		
		,CASE
			WHEN COUNT(DISTINCT CASE WHEN a.CareContDate BETWEEN i.ReportingPeriodStartDate AND i.ReportingPeriodEndDate AND c.CodeProcAndProcStatus IN ('748051000000105','748101000000105','748041000000107','748091000000102','748061000000108','199314001','702545008','1026111000000108','975131000000104') 
					AND a.Unique_CareContactID IS NOT NULL THEN a.Unique_CareContactID ELSE NULL END) < 5
				THEN NULL
			ELSE
				ROUND((COUNT(DISTINCT CASE WHEN a.CareContDate BETWEEN i.ReportingPeriodStartDate AND i.ReportingPeriodEndDate AND c.CodeProcAndProcStatus IN ('748051000000105','748101000000105','748041000000107','748091000000102','748061000000108','199314001','702545008','1026111000000108','975131000000104') 
					AND a.Unique_CareContactID IS NOT NULL THEN a.Unique_CareContactID ELSE NULL END)/5.0),0)*5 
			END AS 'LIApts'

		,CASE
			WHEN COUNT(DISTINCT CASE WHEN a.CareContDate BETWEEN i.ReportingPeriodStartDate AND i.ReportingPeriodEndDate AND c.CodeProcAndProcStatus IN ('1127281000000100','1129471000000105','842901000000108','286711000000107','314034001','449030000','933221000000107','1026131000000100','304891004','443730003') 
					AND a.Unique_CareContactID IS NOT NULL THEN a.Unique_CareContactID ELSE NULL END) < 5
				THEN NULL
			ELSE
				ROUND((COUNT(DISTINCT CASE WHEN a.CareContDate BETWEEN i.ReportingPeriodStartDate AND i.ReportingPeriodEndDate AND c.CodeProcAndProcStatus IN ('1127281000000100','1129471000000105','842901000000108','286711000000107','314034001','449030000','933221000000107','1026131000000100','304891004','443730003') 
					AND a.Unique_CareContactID IS NOT NULL THEN a.Unique_CareContactID ELSE NULL END)/5.0),0)*5			
			END AS 'HIApts'

		,CASE
			WHEN COUNT(DISTINCT CASE WHEN a.CareContDate BETWEEN i.ReportingPeriodStartDate AND i.ReportingPeriodEndDate AND c.CodeProcAndProcStatus IN ('748051000000105','748101000000105','748041000000107','748091000000102','748061000000108','199314001','702545008','1026111000000108','975131000000104','1127281000000100','1129471000000105','842901000000108','286711000000107','314034001','449030000','933221000000107','1026131000000100','304891004','443730003') 
					AND a.Unique_CareContactID IS NOT NULL THEN a.Unique_CareContactID ELSE NULL END) < 5
				THEN NULL
			ELSE
				ROUND((COUNT(DISTINCT CASE WHEN a.CareContDate BETWEEN i.ReportingPeriodStartDate AND i.ReportingPeriodEndDate AND c.CodeProcAndProcStatus IN ('748051000000105','748101000000105','748041000000107','748091000000102','748061000000108','199314001','702545008','1026111000000108','975131000000104','1127281000000100','1129471000000105','842901000000108','286711000000107','314034001','449030000','933221000000107','1026131000000100','304891004','443730003') 
					AND a.Unique_CareContactID IS NOT NULL THEN a.Unique_CareContactID ELSE NULL END)/5.0),0)*5  
			END AS 'HILIApts'

		,CASE
			WHEN COUNT(DISTINCT CASE WHEN a.CareContDate BETWEEN i.ReportingPeriodStartDate AND i.ReportingPeriodEndDate AND a.ConsMechanism = '01' AND a.Unique_CareContactID IS NOT NULL THEN a.Unique_CareContactID ELSE NULL END) < 5
				THEN NULL
			ELSE
				ROUND((COUNT(DISTINCT CASE WHEN a.CareContDate BETWEEN i.ReportingPeriodStartDate AND i.ReportingPeriodEndDate AND a.ConsMechanism = '01' AND a.Unique_CareContactID IS NOT NULL THEN a.Unique_CareContactID ELSE NULL END)/5.0),0)*5  
			END AS 'F2FApts'

		,CASE
			WHEN COUNT(DISTINCT CASE WHEN a.CareContDate BETWEEN i.ReportingPeriodStartDate AND i.ReportingPeriodEndDate AND a.ConsMechanism IN ('02','04','05','09','10','11','12','13','98') AND a.Unique_CareContactID IS NOT NULL THEN a.Unique_CareContactID ELSE NULL END) < 5
				THEN NULL
			ELSE
				ROUND((COUNT(DISTINCT CASE WHEN a.CareContDate BETWEEN i.ReportingPeriodStartDate AND i.ReportingPeriodEndDate AND a.ConsMechanism IN ('02','04','05','09','10','11','12','13','98') AND a.Unique_CareContactID IS NOT NULL THEN a.Unique_CareContactID ELSE NULL END)/5.0),0)*5 
			END AS 'NonF2FApts'

		,CASE
			WHEN COUNT(DISTINCT CASE WHEN a.CareContDate BETWEEN i.ReportingPeriodStartDate AND i.ReportingPeriodEndDate AND a.ConsMechanism IN ('01','02','04','05','09','10','11','12','13','98') AND a.Unique_CareContactID IS NOT NULL THEN a.Unique_CareContactID ELSE NULL END) < 5
				THEN NULL
			ELSE
				ROUND((COUNT(DISTINCT CASE WHEN a.CareContDate BETWEEN i.ReportingPeriodStartDate AND i.ReportingPeriodEndDate AND a.ConsMechanism IN ('01','02','04','05','09','10','11','12','13','98') AND a.Unique_CareContactID IS NOT NULL THEN a.Unique_CareContactID ELSE NULL END)/5.0),0)*5  		
			END AS 'Apts'
		...
		,CASE
			WHEN COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND ReliableImprovement_Flag = 'True' AND Recovery_Flag = 'True' THEN  r.PathwayID ELSE NULL END) < 5
				THEN NULL
			ELSE
				ROUND((COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND ReliableImprovement_Flag = 'True' AND Recovery_Flag = 'True' THEN  r.PathwayID ELSE NULL END)/5.0),0)*5 	 
			END AS 'Count_ReliableRecovery'

		,CASE
			WHEN COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND ReliableImprovement_Flag = 'True' THEN  r.PathwayID ELSE NULL END) < 5
				THEN NULL
			ELSE
				ROUND((COUNT(DISTINCT CASE WHEN CompletedTreatment_Flag = 'True' AND r.ServDischDate BETWEEN i.[ReportingPeriodStartDate] AND i.[ReportingPeriodEndDate] AND ReliableImprovement_Flag = 'True' THEN  r.PathwayID ELSE NULL END)/5.0),0)*5  
			END AS 'Count_Improvement'
...
FROM	[mesh_IAPT].[IDS101referral] r
		...
		LEFT JOIN [Internal_Hierarchies].[Commissioner_Hierarchies_TCUBE] ch ON COALESCE(cc.New_Code, r.OrgIDComm) = ch.Organisation_Code COLLATE database_default --[Reporting].[Ref_ODS_Commissioner_Hierarchies_ICB]
			AND ch.Region_Name <> 'Wales Region' AND (ch.Effective_To IS NULL OR ch.Effective_To >= i.[ReportingPeriodStartDate])
...
--Suppressed version of the DASHBOARD_TTAD_KeyMetricsAvgWaits table not produced due to data suppression issues with the underlying data tables that feed into this table.
